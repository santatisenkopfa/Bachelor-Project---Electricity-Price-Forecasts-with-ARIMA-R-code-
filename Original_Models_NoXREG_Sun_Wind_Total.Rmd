...
```{r}
# === Forecasts med konfidensintervaller ===
# === Træningsdata: 17. januar – 17. februar og forecast for 18. februar ===
train <- spotPrices |> filter(HourDK >= as.POSIXct("2025-01-17") & HourDK < as.POSIXct("2025-02-18"))
train_production <- productionConsumption |> filter(HourDK >= as.POSIXct("2025-01-17") & HourDK < as.POSIXct("2025-02-18"))

# === Fit deterministisk komponent ===
t <- seq(nrow(train))
fit_dhr <- train |> 
  model(dhr = ARIMA(logspotprice ~ PDQ(0,0,0) + pdq(0,0,0) + t +
                    fourier(period = 24, K = 6) +
...
# Fremtidige timer for 18. februar
future_hours <- tibble(HourDK = max(train$HourDK) + hours(1:24)) |> 
  mutate(t = max(t) + 1:24) |> 
  bind_cols(
    as_tibble(fourier(ts(train$logspotprice, frequency = 24), K = 6, h = 24)),
    as_tibble(fourier(ts(train$logspotprice, frequency = 7*24), K = 5, h = 24))
  ) |> as_tsibble(index = HourDK)

# Forecast af deterministisk komponent
future_deterministic <- forecast(fit_dhr, new_data = future_hours) |> 
  as_tibble() |> rename(forecast_deterministic = .mean)

# === Fit SARMA til residualer ===
fit_dhr_fitted <- fitted(fit_dhr)
resid <- train$logspotprice - fit_dhr_fitted$.fitted
fit_SARMA <- auto.arima(resid)

# === Fit modeller med totalproduktion lag 0 og 24 ===
xreg_train <- log(train_production$totalProduction)
model_lag0 <- auto.arima(resid, xreg = xreg_train)

lag24_train <- log(dplyr::lag(train_production$totalProduction, 24))[25:length(xreg_train)]
resid_trim <- resid[25:length(resid)]
model_lag24 <- auto.arima(resid_trim, xreg = lag24_train)

# === Fremtidig totalproduktion for 18. februar ===
future_prod <- productionConsumption |> 
  filter(HourDK >= as.POSIXct("2025-02-18") & HourDK < as.POSIXct("2025-02-19"))
xreg_future_0 <- log(future_prod$totalProduction)

xreg_totalProd_lag24_full <- productionConsumption |> 
  mutate(lagged_total_prod = log(dplyr::lag(totalProduction, 24))) |> 
  select(HourDK, lagged_total_prod)

xreg_future_24 <- xreg_totalProd_lag24_full |> 
  filter(HourDK >= as.POSIXct("2025-02-18") & HourDK < as.POSIXct("2025-02-19")) |> 
  pull(lagged_total_prod)

# === Forecasts ===
forecast_0 <- forecast::forecast(model_lag0, xreg = xreg_future_0, h = 24)
forecast_24 <- forecast::forecast(model_lag24, xreg = xreg_future_24, h = 24)

forecast_0_df <- tibble(
  HourDK = future_hours$HourDK,
  forecast_random = as.numeric(forecast_0$mean),
  lower_0 = as.numeric(forecast_0$lower[, 2]),
  upper_0 = as.numeric(forecast_0$upper[, 2])
)

forecast_24_df <- tibble(
  HourDK = future_hours$HourDK,
  forecast_random = as.numeric(forecast_24$mean),
  lower_24 = as.numeric(forecast_24$lower[, 2]),
  upper_24 = as.numeric(forecast_24$upper[, 2])
)

# === Kombiner med deterministisk komponent ===
combined_0 <- bind_cols(future_deterministic, forecast_0_df[c("forecast_random", "lower_0", "upper_0")]) |> 
  mutate(
    forecast_total_lag0 = exp(forecast_deterministic + forecast_random),
    lower_bound_0 = exp(forecast_deterministic + lower_0),
    upper_bound_0 = exp(forecast_deterministic + upper_0)
  )

combined_24 <- bind_cols(future_deterministic, forecast_24_df[c("forecast_random", "lower_24", "upper_24")]) |> 
  mutate(
    forecast_total_lag24 = exp(forecast_deterministic + forecast_random),
    lower_bound_24 = exp(forecast_deterministic + lower_24),
    upper_bound_24 = exp(forecast_deterministic + upper_24)
  )

# === Faktiske priser 18. februar ===
actual <- spotPrices |> 
  filter(HourDK >= as.POSIXct("2025-02-18") & HourDK < as.POSIXct("2025-02-19")) |> 
  select(HourDK, actual = SpotPriceDKK)

# === Samlet forecast-dataframe ===
forecast_df <- combined_0 |> select(HourDK, forecast_total_lag0, lower_bound_0, upper_bound_0) |> 
  left_join(combined_24 |> select(HourDK, forecast_total_lag24, lower_bound_24, upper_bound_24), by = "HourDK") |> 
  left_join(actual, by = "HourDK")

# === Plot ===
ggplot(forecast_df, aes(x = HourDK)) +
  geom_ribbon(aes(ymin = lower_bound_0, ymax = upper_bound_0), fill = "blue", alpha = 0.2) +
  geom_ribbon(aes(ymin = lower_bound_24, ymax = upper_bound_24), fill = "forestgreen", alpha = 0.2) +
  geom_line(aes(y = actual, color = "Actual Price"), linewidth = 1.2) +
  geom_line(aes(y = forecast_total_lag0, color = "Forecast with Total Production (lag 0h)"), linewidth = 1.2) +
  geom_line(aes(y = forecast_total_lag24, color = "Forecast with Total Production (lag 24h)"), linewidth = 1.2) +
  scale_color_manual(values = c(
  ...
  labs(
    title = "Forecast vs. Actual Electricity Spot Price (Feb 18)",
    x = "Hour",
    y = "Electricity Price (DKK)",
    color = "Legend"
  ) +
...
valid_df <- forecast_df |> filter(
  !is.na(actual),
  !is.na(forecast_total_lag0),
  !is.na(forecast_total_lag24)
)

rmse_lag0 <- rmse(valid_df$actual, valid_df$forecast_total_lag0)
mape_lag0 <- mape(valid_df$actual, valid_df$forecast_total_lag0)

rmse_lag24 <- rmse(valid_df$actual, valid_df$forecast_total_lag24)
mape_lag24 <- mape(valid_df$actual, valid_df$forecast_total_lag24)

cat("=== Forecast Evaluation (Feb 18) ===\n")
cat("Lag 0:\n  RMSE = ", round(rmse_lag0, 2), "\n  MAPE = ", round(mape_lag0, 2), "%\n\n")
cat("Lag 24:\n  RMSE = ", round(rmse_lag24, 2), "\n  MAPE = ", round(mape_lag24, 2), "%\n")


```

```{r}
# === Træningsdata: 16. januar – 16. februar og forecast for 17. februar ===
train <- spotPrices |> filter(HourDK >= as.POSIXct("2025-01-16") & HourDK < as.POSIXct("2025-02-17"))
train_production <- productionConsumption |> filter(HourDK >= as.POSIXct("2025-01-16") & HourDK < as.POSIXct("2025-02-17"))
...
# Fremtidige timer for 17. februar
future_hours <- tibble(HourDK = max(train$HourDK) + hours(1:24)) |> 
  mutate(t = max(t) + 1:24) |> 
  bind_cols(
    as_tibble(fourier(ts(train$logspotprice, frequency = 24), K = 6, h = 24)),
...
# === Fremtidig totalproduktion for 17. februar ===
future_prod <- productionConsumption |> 
  filter(HourDK >= as.POSIXct("2025-02-17") & HourDK < as.POSIXct("2025-02-18"))
xreg_future_0 <- log(future_prod$totalProduction)
...
ggplot(forecast_df, aes(x = HourDK)) +
  ...
  labs(
    title = "Forecast vs. Actual Electricity Spot Price (Feb 17)",
    x = "Hour",
    y = "Electricity Price (DKK)",
    color = "Legend"
  ) +
...
cat("=== Forecast Evaluation (Feb 17) ===\n")
cat("Lag 0:\n  RMSE = ", round(rmse_lag0, 2), "\n  MAPE = ", round(mape_lag0, 2), "%\n\n")
cat("Lag 24:\n  RMSE = ", round(rmse_lag24, 2), "\n  MAPE = ", round(mape_lag24, 2), "%\n")
...
```{r}
# === Træningsdata: 15. januar – 15. februar og forecast for 16.–18. februar ===
train <- spotPrices |> filter(HourDK >= as.POSIXct("2025-01-15") & HourDK < as.POSIXct("2025-02-16"))
train_production <- productionConsumption |> filter(HourDK >= as.POSIXct("2025-01-15") & HourDK < as.POSIXct("2025-02-16"))
...
# Fremtidige timer: 72 timer = 3 dage
future_hours <- tibble(HourDK = max(train$HourDK) + hours(1:72)) |> 
  mutate(t = max(t) + 1:72) |> 
  bind_cols(
    as_tibble(fourier(ts(train$logspotprice, frequency = 24), K = 6, h = 72)),
    as_tibble(fourier(ts(train$logspotprice, frequency = 7*24), K = 5, h = 72))
  ) |> as_tsibble(index = HourDK)
...
# === Fremtidig totalproduktion for 16.–18. februar ===
future_prod <- productionConsumption |> 
  filter(HourDK >= as.POSIXct("2025-02-16") & HourDK < as.POSIXct("2025-02-19"))
...
# === Forecasts ===
forecast_0 <- forecast::forecast(model_lag0, xreg = xreg_future_0, h = 72)
forecast_24 <- forecast::forecast(model_lag24, xreg = xreg_future_24, h = 72)
...
ggplot(forecast_df, aes(x = HourDK)) +
  geom_ribbon(aes(ymin = lower_bound_0, ymax = upper_bound_0), fill = "blue", alpha = 0.2) +
  geom_ribbon(aes(ymin = lower_bound_24, ymax = upper_bound_24), fill = "forestgreen", alpha = 0.2) +
  geom_line(aes(y = actual, color = "Actual Price"), linewidth = 1.1) +
  geom_line(aes(y = forecast_total_lag0, color = "Forecast with Total Production (lag 0h)"), linewidth = 1.1) +
  geom_line(aes(y = forecast_total_lag24, color = "Forecast with Total Production (lag 24h)"), linewidth = 1.1) +
  scale_color_manual(values = c(
    "Actual Price" = "black",
    "Forecast with Total Production (lag 0h)" = "blue",
    "Forecast with Total Production (lag 24h)" = "forestgreen"
...
cat("=== Forecast Evaluation (Feb 16–18) ===\n")
cat("Lag 0:\n  RMSE = ", round(rmse_lag0, 2), "\n  MAPE = ", round(mape_lag0, 2), "%\n\n")
cat("Lag 24:\n  RMSE = ", round(rmse_lag24, 2), "\n  MAPE = ", round(mape_lag24, 2), "%\n")
