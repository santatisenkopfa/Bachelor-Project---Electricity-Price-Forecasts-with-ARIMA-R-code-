...
ggplot(forecast_comparison_3days, aes(x = HourDK)) +
  ...
  labs(
    title = "Forecast vs. Actual Electricity Spot Price (Feb 16–19)",
    x = "Hour",
    y = "Electricity Price (DKK)",
    color = "Legend"
  ) +
...
# Genfit SARMA-modellen og behold modelobjektet
model_SARMA <- fit_dhr |>
  model(sarma = ARIMA(filtreret ~ 1 + PDQ(1,1,1, period = 24) + pdq(1,0,1)))
...
```{r}
#Gennemsnitlig produktion fra solenergi
# Opret dato søjle fra HourUTC
productionConsumption$HourUTC <- as.POSIXct(productionConsumption$HourUTC, tz = "UTC")
productionConsumption$Date <- as.Date(productionConsumption$HourUTC)

# Opret en total solproduktion søjle
productionConsumption <- productionConsumption %>%
  mutate(TotalSolarMWh = SolarPowerLt10kW_MWh +
                         SolarPowerGe10Lt40kW_MWh +
                         SolarPowerGe40kW_MWh +
                         SolarPowerSelfConMWh)

#Gennemsnitlig timeproduktion 15.06.2024-15.07.2024
period_avg <- productionConsumption %>%
  filter(Date >= as.Date("2024-06-15") & Date <= as.Date("2024-07-15")) %>%
  summarise(avg_hourly_solar = mean(TotalSolarMWh, na.rm = TRUE)) %>%
  pull(avg_hourly_solar)

# Print resultat
cat("Average hourly solar production from June 15 to July 15, 2024:", round(period_avg, 2), "MWh\n")

#Gennemsnitlig timeproduktion 15.01.2025-15.02.2025
period_avg <- productionConsumption %>%
  filter(Date >= as.Date("2025-01-15") & Date <= as.Date("2025-02-15")) %>%
  summarise(avg_hourly_solar = mean(TotalSolarMWh, na.rm = TRUE)) %>%
  pull(avg_hourly_solar)

# Print resultat
cat("Average hourly solar production from January 15 to February 15, 2025:", round(period_avg, 2), "MWh\n")
```

```{r}
# === Træningsdata: 16. januar – 16. februar ===
...
combined_0 <- bind_cols(future_deterministic, forecast_0_df["forecast_random"]) |> 
  mutate(forecast_total_lag0 = exp(forecast_deterministic + forecast_random))

combined_24 <- bind_cols(future_deterministic, forecast_24_df["forecast_random"]) |> 
  mutate(forecast_total_lag24 = exp(forecast_deterministic + forecast_random))

# === Faktiske elpriser 17. februar ===
actual <- spotPrices |> 
  filter(HourDK >= as.POSIXct("2025-02-17") & HourDK < as.POSIXct("2025-02-18")) |> 
  select(HourDK, actual = SpotPriceDKK)

# === Saml og plot ===
forecast_df <- combined_0 |> select(HourDK, forecast_total_lag0) |> 
  left_join(combined_24 |> select(HourDK, forecast_total_lag24), by = "HourDK") |> 
...
# === Beregn RMSE og MAPE ===
library(Metrics)  # indeholder rmse() og mape()
...
cat("Lag 0:\n")
cat("  RMSE  = ", round(rmse_lag0, 2), "\n")
cat("  MAPE  = ", round(mape_lag0, 2), "%\n\n")

cat("Lag 24:\n")
cat("  RMSE  = ", round(rmse_lag24, 2), "\n")
cat("  MAPE  = ", round(mape_lag24, 2), "%\n")
...
```{r}
# === Træningsdata: 17. januar – 17. februar ===
